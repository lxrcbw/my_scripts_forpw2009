...
const rlurl2Arr = [], rlheaderArr = [],rlbodyArr = []
let rlurl2 = $.getdata('rlurl2')
...
if ($.isNode()) {
   if (process.env.rlurl2 && process.env.rlurl2.indexOf('#') > -1) {
   rlurl2 = process.env.rlurl2.split('#');
   console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.rlurl2 && process.env.rlurl2.indexOf('\n') > -1) {
   rlurl2 = process.env.rlurl2.split('\n');
...
if($request&&$request.url.indexOf("Common/pvlog")>=0) {
   const rlurl2 = $request.url.split('?')[1]
   if(rlurl2)     $.setdata(rlurl2,`rlurl2${status}`)
   $.log(`[${Ariszy}] 获取rlurl2请求: 成功,rlurl2: ${rlurl2}`)
   $.msg(`rlurl2${status}: 成功🎉`, ``)
...
let url = rlurl2.replace(/&video_id=\d+/,'')
...
let url = rlurl2.replace(/\d{5}$/,`${videoid}`)
 return new Promise((resolve) => {
...
let accesstoken = rlurl2.match(/access_token=\w{32}/)
 return new Promise((resolve) => {
    let video_info_url = {
...
async function wiTask(){
 return new Promise((resolve) => {
    let wiTask_url = {
   		url: `https://ranlv.lvfacn.com/api.php/Share/wiTask?${rlurl2}`,
        headers: JSON.parse(rlheader),
    	}
...
let user_token = rlurl2.match(/user.*?(?=&)/)+''
let access_token = rlurl2.match(/access_token=\w+/)+''
...
let url = rlurl2.replace(/&video_id=\d{5}/,``)
 return new Promise((resolve) => {
    let mySupport_url = {
...
let url = rlurl2.replace(/\d{5}$/,`${supportvideoid}`)
 return new Promise((resolve) => {
    let goVote_url = {
...
let user_token = rlurl2.match(/user_token=\w+.\w+.\w+/)+''
let access_token = rlurl2.match(/access_token=\w+/)+''
let new_user_token = user_token.replace(/user_token=/,'')
let new_access_token = access_token.replace(/access_token=/,'')
 return new Promise((resolve) => {
